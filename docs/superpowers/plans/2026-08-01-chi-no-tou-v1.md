...
- **「書いた」= 自分のNotionのレコードのみ**（allowlist方式: `owner === 'personal'` のみ積む。team・サブスク由来=`owner === 'subscription'` は積まない。読んだはどれでも積む）
- `prefers-reduced-motion` は全演出静止。ダークモードは Tailwind `dark:` で夜空版
- 1歩=1mm（`MM_PER_STEP`・調整可能定数）。くすみ日数 `DULL_DAYS = 90`（調整可能定数）
- **v1はオーナーの暗転リリース**: 表示・記録・取込はすべて `isTowerEnabled()`（`src/lib/tower-flags.ts`）の端末のみ。中身は既存の先行体験フラグ `getSettings()?.earlyAccess === true` の再利用（/api/premium/status → PremiumSync → settings が既に配線済み）。開通は /admin台帳の early_access か Vercel の `EARLY_ACCESS_EMAILS`。全体公開はこの1関数の変更のみ。⚠️マルチ部署検索の先行体験を他ユーザーへ開く時は塔専用フラグに分離すること
...
| Create `src/lib/tower-flags.ts` | 開放判定の単一チョークポイント（v1=先行体験フラグの端末のみ） |
| Create `src/lib/tower-backfill.ts` | バックフィルのseam純関数（buildBackfillRequest/applyBackfill・as-built追加） |
...
- Produces:
  ...
  - `type TowerState = { steps: Step[]; lastSeenSteps: number; lastSeenAt: string; backfilledAt: string }`
  ...
  - `ingestRecords(state, hits): TowerState`（`owner==='personal'` のみ取込＝team/サブスクを除外・`createdAt` を歩の日時に）
...
- [ ] **Step 1: 開放判定と `quiz-srs.ts` の読み取り口を足す**

`src/lib/tower-flags.ts` を新規作成:
...
（`tower-steps.ts` の中では判定しない——純関数とテストをゲートから独立させるため、判定は各フックの呼び出し側で行う。）
...
```bash
git add src/lib/tower-flags.ts src/lib/quiz-srs.ts src/components/reader/ReaderMarksProvider.tsx src/components/QuizCard.tsx src/components/DailyQuestionCard.tsx
git commit -m "知の塔: イベント配線（読んだ=リーダー開閲・即答/磨き直し=クイズok。判定は記録より先・先行体験ゲート）"
```
...
> **As-built note (2026-08-01 最終レビュー後)**: 下記スニペットのバックフィル部は実装時に変更した。`{keyword:'', mode:'search', pageSize:1000}` は実APIと不整合（トークン必須400・空keywordは空配列200・pageSize上限100）のため、`src/lib/tower-backfill.ts` の純関数（`buildBackfillRequest`=settings由来トークン+`mode:'recent'`／`applyBackfill`=ingest+backfilledAt刻印+水位上げ）で **最新50件の組み上げ＋残りは日常のpassive ingestで漸増** に確定。保存は fetch 解決後に `loadTowerState()` を merge base として updater 外で行う（並行書込を握り潰さない・StrictMode純度）。`useBodyScrollLock` は無引数。オーバーレイのマウントは `towerOpen && isTowerEnabled()`（Task 8）。
...
1. 検索タブにカードが出る（初回は検索結果のpassive ingestで歩が積まれてから。先行体験フラグの端末のみ）
2. フル画面: 初回バックフィルで自分の最新50件が積まれている（ゼロスタートしていない。残りはpassive ingestで漸増——as-built note参照）
...
12. 先行体験フラグの無いアカウント（シークレットウィンドウ等）ではカードが出ず、読んでも・クイズに答えても `medinode_tower_v1` に何も書かれない
...
- ゲート（`isTowerEnabled`）が表示・記録・取込の全経路を覆っているか（フラグ無し端末では localStorage に一切書かれない）
...
| 暗転リリース（v1はオーナーのみ・先行体験フラグ再利用） | Task 4（tower-flags.ts）・6・8 のゲート |
