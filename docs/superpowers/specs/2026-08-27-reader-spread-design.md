...
## 最終目標（2026-08-27 オーナー確定・追記）

**誌面の完成度の基準は、パイロット誌面（アーティファクト 6bd2b9af-4e85-4a33-8cc9-ddfcff829e85・酸素療法）そのもの。**
第1版の骨格で止めず、パイロットの見た目まで寄せていく。同日、表示側に以下を実装した
（保存形・APIは不変。詳細は memory-vault `reader-spread-pilot-fidelity`）:
節末recap「この節の答え」／昇格ブロックの深掘り除外／出典サマリ／フローの縦導線＋条件行／
Go/No-Goのラベル指定と赤アクセント／部品内の数値ブランド強調／gauge部品／extraParts（1節複数部品）／
entries「いまの状況から探す」／⚡要点の2件先出し／🔖の線画変換／devハーネス `/dev/spread`。
残り: バッジ行（要プロパティ供給）・編集レイヤー（段4）・現在地ナビの常時チップ型。
