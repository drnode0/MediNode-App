...
| ③ note特典コード（TRIAL_CODES） | 14日→**30日** | `trialCodeDays()` が期間で自動切替（env `TRIAL_DAYS` が優先） |
| ④ 友達紹介（常設・新設） | 新規**30日**・紹介者**+14日** | `REFERRAL_NEW_USER_DAYS` / `REFERRAL_REWARD_DAYS` |

- 有料note購入者（③）が「カードなし最長」の頂点。カード登録だけの人（②14日）より明確に上位に保ち、初期購入者を裏切らない。
- キャンペーン終了（8/19 JST 0:00）で①③は自動で通常値へ戻る（コード変更・再デプロイ不要）。②は14日のまま恒久。
...
- 不正対策: 自分のコード不可／受け取りは生涯1回（`referred_user_id` UNIQUE）／Stripe決済歴・comp保持者は新規側になれない／紹介者への還元は10人まで（超過後も新規側30日は有効）／既存レート制限。
...
3. [ ] Vercel 環境変数:
   - `TRIAL_DAYS` … **削除**（コードが期間で 30→14 を自動切替。設定したままだとキャンペーンが効かない）
...
5. [ ] 動作確認: 新規登録→7日付与／noteコード→30日／紹介コード発行→別アカウントで償還→30日＋紹介者延長／/admin のKPI表示
