...
import { CLUSTER_PITCH } from './field-cluster'
import { paletteOf } from '@/lib/recall/field-palette'
import { isDarkNow } from '@/components/recall/useIsDark'
...
type Props = {
  ...
  mode?: 'ring' | 'cluster'
  free?: boolean            // 近景の縦回しの頭打ちを外す
  lockNear?: boolean        // 近景から出られない（背景タップ・ホイール・ピンチで中景へ行かない）
  show?: FieldFrameArgs['show']
  familyLabels?: FieldFrameArgs['familyLabels']
  onPlanetTap?: (slot: number) => void
  onFamilyTap?: (kind: string) => void
  fanOnTouch?: boolean      // 触れたときだけ扇形を出す（案4）
...
export const RecallField = forwardRef<FieldHandle, Props>(function RecallField(props, ref) {
  ...
  const midSlot = useRef<number | null>(null)
  const farFocus = useRef<{ at: [number, number, number]; zoom: number } | null>(null)
  const familyFocus = useRef<{ kind: string; until: number } | null>(null)
  ...
  const goStage = (next: FieldStage, slot: number | null) => {
    ...
    vel.current = 0
    startFly(cameraFor(cam.current, P.center, next, seatOf(next === 'near' ? nearSlot.current : midSlot.current), P.mode ?? 'ring'), FLY_MS)
    stage.current = next
    P.onStage(next, next === 'near' ? nearSlot.current : midSlot.current)
    return true
  }
  ...
  useEffect(() => {
    ...
    const initial = initialCamera(seats, props.mode ?? 'ring')
    const nearSeat = props.initialNear !== undefined
      ? seats.find((s) => s.slot === props.initialNear && s.n > 0) ?? null
      : null
    if (nearSeat) {
      cam.current = cameraFor(initial, props.center, 'near', nearSeat, props.mode ?? 'ring')
      stage.current = 'near'
      nearSlot.current = nearSeat.slot
      enteredAt.current = performance.now()
      return
    }
    cam.current = props.initialStage === 'far' ? cameraFor(initial, props.center, 'far', null, props.mode ?? 'ring') : initial
    if (props.initialStage === 'far') stage.current = 'far'
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      hits.current = drawField(ctx, {
        ...
        show: P.fanOnTouch ? { ...(P.show ?? {}), fans: true, fanAlpha: Math.max(0, Math.min(1, (2600 - (now - touchedAt.current)) / 600)) } : P.show,
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (next !== stage.current) { stage.current = next; if (next === 'far') midSlot.current = null; P.onStage(next, null) }
    }
  ...
  const onUp = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (stage.current === 'far') {
      ...
      if (fam && fl) {
        farFocus.current = { at: [fl.at[0], fl.at[1] - 0.2, fl.at[2]], zoom: 3.4 }
        familyFocus.current = { kind: fam.kind, until: performance.now() + 3200 }
        fly.current = null
        startFly({ ...cam.current, focus: [...farFocus.current.at], zoom: 3.4 }, FLY_MS)
    ...
    if (slot === null && stage.current === 'far' && farFocus.current) {
      ...
      startFly({ ...cam.current, focus: [0, 0, 0], zoom: cameraFor(cam.current, P.center, 'far', null, 'cluster').zoom }, FLY_MS)
      return
    }
    if (slot !== null) {
      if (P.mode === 'cluster') {
        if (stage.current === 'mid' && midSlot.current === slot) { if (P.onPlanetTap) P.onPlanetTap(slot); else goStage('near', slot) }
        else goStage('mid', slot)
        return
      }
      if (P.onPlanetTap) P.onPlanetTap(slot); else goStage('near', slot)
    }
  }
