...
追補4: §22〜§23（2026-08-03 初心者目線の文言・動線レビューで見つかった10件と、段B-2の確定スコープ）を追加。**§21は§23で置き換わる**
...
# 追補4（2026-08-03・初心者目線の文言・動線レビュー）

段B-1を本番へ出し、オーナーが実機で通した直後に、**慣れないユーザーの目線で文言と動線を通しで読み直した**。穴が10件見つかった。**オーナー判断：すべて段B-2で直す**（段B-1には追い足さない）。

以下は「見つけた穴」と「どう直すか」の記録であり、段B-2の作業項目そのものである。
...
いまの完了ページには「Notionとの接続を確認しました」と「閉じれば何も保存されません」が同居し、戻っても何も起きず、下に「自動的な引き継ぎには今のところ対応していません」と書いてある。初心者は**「結局、接続できたのか」**が判断できず、次の行動もどこにも書かれていない。

直し方：段B-2で自動引き取り（§3c）が入れば「MediNodeに戻ると、読み取るDBを選べます」が真になる。**文言の書き戻しを自動引き取りと同じコミットで行う**こと。片方だけ入れると嘘が残る。

**② PCハンドオフの制限時間（`PENDING_TTL_MS = 10分`）が短すぎる**

この10分は「中間ページを開いてから、Notionで許可を押し終えるまで」。実際にやることは、リンクをコピー → PCを開く → ブラウザ起動 → Notionにログイン → ページを選ぶ。10分では足りない。切れると「この接続リンクは使えません」としか出ない。

しかもiPhoneでは**ハンドオフが主経路だと実機で確定した**（§5 #1の結果：Notionアプリが横取りする）。**最頻経路がいちばん時間切れしやすい**構造になっている。

直し方：`PENDING_TTL_MS` を **30分**にする。stateは192ビット・一回限り・一方向なので、TTLは主防御ではない。

**③ コピーしたリンクをPCへどう渡すか書いていない**

「リンクをコピー」の直下に「このリンクはあなた専用です。他の人に送らないでください」とあるため、**自分宛にメールやLINEで送っていいのかで迷う**。渡す手段が書かれていない。

直し方：「自分のパソコンへ送る分には問題ありません（自分宛のメールやAirDropなど）。他の人には送らないでください」と、許される渡し方を明示する。

### 22b. 迷う

**④ どのページを選べばいいか書いていない** — §19cで文言は確定済み。中間ページに入れる。

**⑤「うまく開かないときは、パソコンで」が条件付きの見出しのまま** — iPhoneでは既定でうまく開かない。`NEXT_PUBLIC_EASY_CONNECT_MOBILE=handoff` に倒し（§12）、見出しも「パソコンで続ける」のように条件を外す。いまは最頻経路が灰色の小さい字になっている。

**⑥「次の画面」がNotionの画面だと言っていない** — 押すと突然Notionのブランド画面が出る。「Notionの画面が開きます」と先に言う。

### 22c. 矛盾している

**⑦ カードの「未ログインの場合は案内が出ます」が実装と食い違う** — 現在の `start` は理由を出さずに黙ってホームへ戻す。案内は出ない。さらに§9の登録先行が入れば未ログインでカードに到達しないため、**この一文ごと削除**する。

**⑧ エラー文言「アプリからもう一度お試しください」が行き先を示していない** — 初心者は入口を知らない。「MediNodeの設定 →〈Notion接続〉から」まで書く。

### 22d. 入口が無い

**⑨ 手動接続の既存ユーザーが、かんたん接続へ乗り換える入口が無い**

設定にあるのは「かんたん接続でつながっています」＝**すでにOAuthの人向け**だけ。手動Tokenで運用している既存ユーザーには入口が一切ない。セットアップウィザードは初回しか通らないため、**既存ユーザーは永久に乗り換えられない**。

直し方：設定 → Notion接続設定に、`notionAuthKind !== 'oauth'` の人向けの導線を置く。§10bの保護（旧トークン退避・可読性検査・conflict）がそのまま効くので、押しても壊れない。

**⑩ 中間ページに離脱路が無い** — 正常系の中間ページには「やめる」導線がない。エラー画面にしか「MediNodeに戻る」が無い。認可へ進むしかないように見える。

## 23. 段B-2のスコープ（§21を置き換え）

§21に22の是正を織り込んだ確定版。**文言と機能は同じコミットで入れる**（片方だけだと嘘が残るため）。

1. **アプリ起動時の引き取り**（`claimable` → `claim`）。`hadServerSettings === false` のときは置き換えでなくマージ（§10d）／完了ページの文言を①に合わせて書き戻す
2. **`OAuthFinish` の作り直し** — conflict フェーズ ＋ §20cの穴の是正（読めないDBを黙って保存しない）
3. **設定からの「読み取るDBを選び直す」**（§19b・再認可なし）
4. **設定からの乗り換え入口**（⑨）と「元の接続に戻す」（§10b step 5）
5. **かんたん接続カードの2状態表示**（⑦の一文を削除）
6. **中間ページの文言と構成**（③④⑤⑥⑩）＋ `NEXT_PUBLIC_EASY_CONNECT_MOBILE=handoff` の既定化を検討
7. **`PENDING_TTL_MS` を30分へ**（②）
8. **エラー文言に行き先を書く**（⑧）
9. テレメトリ（§14）と /admin 表示
