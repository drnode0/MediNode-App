...
// 何がどこまで進んでいるかを、記事ごとに1目で読めるようにするための段。
...
export function SpreadCard() {
  ...
  // サブスク用DBの一覧が引けなかった。未投入の行を出せていないことを画面に出す
  ...
  // 一覧の1行。busy・armed・run・reviewQuiz などをそのまま使うので、コンポーネントの中に置く。
      ...
      return (
        ...
        >
          <span className="basis-full flex items-baseline gap-2 min-w-0">
            ...
            <code className="text-[11px] text-gray-400 dark:text-gray-500 shrink-0" title={item.pageId}>
  ...
  return (
    <section className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 mb-4">
      ...
      {rows !== null && listed.length === 0 && !loadFailed && (
