...
// ⚠️ ポーズを取る文は落ちる。ただし数字を並べるのも違った（2026-08-03オーナー裁定）——
// 寸法は画面の朱線がすでに持っているので、文言では繰り返さない。
// 文言の役目は弧を描くこと: 日々の学び → 葉 → 越えていく → そして問いを開いたまま渡す。
// 最後の一行に答えを書かない。答えは出口のボタン（今日の1問）で自分が出す。
// 敬体と二人称は使わない（六つの禁）——「〜します／でしょうか」は常体へ、「あなた」は「じぶん」へ。
