...
// 1つの主張に付けられる伏せ字の上限。検出（holes.ts）・保存時の検査（管理APIの PATCH）・
// 管理画面の説明文と穴の追加操作が同じ数を見る。定義はこの軽いファイルに置く。
// holes.ts は評価時に多数の正規表現を組み立てるので、数だけが要る管理画面（クライアント側）が
// この定数のために holes.ts を読み込むと、検出規則一式がブラウザのバンドルに入ってしまう。
