...
//
// PGroonga（migration 0031・任意）は、いまは使っていない。段0の検索は
// active な主張を毎回すべて読み、順位も足切りもこの関数だけが決める
// （search/route.ts。索引の有無で結果が変わらないことを優先している）。
// 0031 は主張が増えて全件読みが実際の負担になったときのために取ってあるだけで、
// 流さなくても段0は同じ結果で動く。引き直しの目安は設計書の再検討ライン
// （主張が 2,000 を超えたとき）。使い始めるときも、絞るのは候補までで、
// 順位と足切りはこの関数に残す（設計時の実測がそのまま本番の振る舞いになる）。
