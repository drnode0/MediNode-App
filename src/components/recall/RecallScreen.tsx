  ...
  const seatButton = (s: { slot: number; label: string; n: number; escaping: number }) => (
    <li key={s.slot}>
      <button type="button" onClick={() => field.current?.jumpTo(s.slot)}
        ...
        className={`whitespace-nowrap rounded px-2 py-1 text-[11.5px] ${hereSlot === s.slot ? 'bg-amber-200/90 text-slate-900' : 'text-slate-400 hover:text-slate-100 hover:bg-white/10'}`}>
        {s.label}
        {s.n > 0 && <span className="ml-1.5 opacity-60 tabular-nums">{s.n}</span>}
        {s.escaping > 0 && <span className={`ml-1.5 tabular-nums ${hereSlot === s.slot ? 'text-amber-800' : 'text-amber-200'}`}>●{s.escaping}</span>}
      </button>
    </li>
  )
  ...
  return (
    ...
    <div className="fixed inset-0 z-0 bg-[#0B1524] text-slate-100 overflow-hidden" style={{ fontFamily: '"Zen Kaku Gothic New",-apple-system,"Hiragino Sans",sans-serif' }}>
      ...
      <div className="absolute left-7 pointer-events-none" style={{ top: headerH + 24 }}>
        ...
        <p className="mt-1.5 text-[11px] font-light tracking-[.08em] text-slate-400">検証済みの主張 {data.claims.length}　中心に近いほど、自分のもの</p>
      </div>
      <div className="absolute right-7 text-right pointer-events-none" style={{ top: headerH + 28 }}>
        ...
        <p className="text-[10.5px] text-slate-400 tracking-[.1em] mt-1">残した {data.counts.kept} ／ 深く残した {data.counts.settled} ／ 読んだ {data.counts.touched}</p>
      </div>

      {/* いま見ているジャンル。近景では族名と内訳まで出す。 */}
      {herePlanet && (
        <div className="absolute left-1/2 -translate-x-1/2 flex flex-wrap gap-x-3 gap-y-1 items-baseline justify-center pointer-events-none max-w-[92%]" style={{ top: headerH + 22 }}>
          ...
          <span className="text-[11.5px] text-slate-400">{CORE_LABEL[herePlanet.seat.kind]}</span>
          {stage === 'near' && (
            <>
              <span className="text-[11.5px] text-slate-400">主張 {herePlanet.seat.n} ・ 残した {hereStat.kept} ・ 深く残した {hereStat.settled}</span>
              {hereStat.escaping > 0 && <span className="text-[11.5px] text-amber-200">離れかけ {hereStat.escaping}</span>}
            </>
          )}
        </div>
      )}

      <div className="absolute left-7 bottom-[124px] text-[10.5px] text-slate-400 leading-7 tracking-[.06em] pointer-events-none max-[860px]:hidden">
        <div><i className="inline-block w-2 h-2 rounded-full mr-2 align-[1px]" style={{ background: '#F4F7FA', boxShadow: '0 0 8px #F4F7FA' }} />深く残した（輪の内側）</div>
        <div><i className="inline-block w-2 h-2 rounded-full mr-2 align-[1px]" style={{ background: '#EBF2FB' }} />残した（明るいほど思い出せる）</div>
        <div><i className="inline-block w-2 h-2 rounded-full mr-2 align-[1px]" style={{ background: '#F6E7B8', boxShadow: '0 0 8px #F6E7B8' }} />離れかけ（外縁を割る）</div>
        <div><i className="inline-block w-2 h-2 rounded-full mr-2 align-[1px]" style={{ background: '#8FA3BD' }} />読んだ　<i className="inline-block w-2 h-2 rounded-full mx-2 align-[1px]" style={{ background: '#7C8DA6' }} />未着手</div>
      </div>

      {shelf.length > 0 && !card && (
        <div className={`absolute left-1/2 -translate-x-1/2 ${pill ? 'bottom-[196px]' : 'bottom-[160px]'} text-[11px] tracking-[.1em] text-slate-400 pointer-events-none`}>
          離れた主張が <b className="text-amber-200 font-medium tabular-nums">{shelf.length}</b>　棚をタップで開く
        </div>
      )}
      {pill && <div className="absolute left-1/2 -translate-x-1/2 bottom-[160px] max-w-[90%] text-[12px] tracking-[.06em] text-cyan-100 bg-[rgba(12,20,30,.92)] border border-slate-600/40 rounded-full px-4 py-2 pointer-events-none">{pill}</div>}

      {/* 帯。主張のある席が先頭で、空の席は末尾に畳む。離れかけの数を光の色で添える。 */}
      <nav aria-label="ジャンル" className="absolute left-3 right-3 bottom-[62px] rounded-[10px] border border-slate-600/40 bg-[rgba(22,41,63,.92)] overflow-hidden backdrop-blur">
        <ul className="flex gap-1 m-0 px-2 py-1.5 list-none overflow-x-auto">
          <li>
            <button type="button" onClick={sweep}
              className="whitespace-nowrap rounded px-2 py-1 text-[11.5px] text-amber-200 hover:bg-white/10">すべて</button>
          </li>
          {full.map(seatButton)}
          {empty.length > 0 && (
            <li>
      ...
      <div className="absolute left-1/2 -translate-x-1/2 bottom-3 flex gap-2.5 items-center">
        <button type="button" onClick={() => check(hereSlot)}
          className="rounded-full border border-amber-200/70 text-amber-100 bg-[rgba(12,20,30,.9)] px-5 py-[11px] text-[12.5px] tracking-[.08em] hover:bg-amber-200/10 backdrop-blur">この惑星を確かめる</button>
        <div className="flex rounded-full border border-slate-600/40 bg-[rgba(12,20,30,.9)] overflow-hidden">
          <button type="button" onClick={() => pickCenter('outside')} className={`px-3.5 py-[11px] text-[11.5px] ${center === 'outside' ? 'text-amber-200' : 'text-slate-400'}`}>外から</button>
          <button type="button" onClick={() => pickCenter('inside')} className={`px-3.5 py-[11px] text-[11.5px] ${center === 'inside' ? 'text-amber-200' : 'text-slate-400'}`}>中心から</button>
        </div>
        {stage === 'near' && <button type="button" onClick={() => field.current?.backToMid()} className="rounded-full border border-slate-600/40 bg-[rgba(12,20,30,.9)] px-4 py-[11px] text-[12px] tracking-[.08em] text-slate-300">戻る</button>}
        {shelf.length > 0 && <button type="button" onClick={reset} className="rounded-full border border-slate-600/40 bg-[rgba(12,20,30,.9)] px-5 py-[11px] text-[12.5px] tracking-[.08em]">戻す</button>}
      </div>
