...
読者から投稿された臨床疑問がナレッジ化されたことを知らせる機能。
常設タブは増やさず、**起動時バナー（×で既読化・プレミアム会員のみ）＋設定内の一覧**で完結させる。
一覧は**非プレミアムにも公開**する（「この疑問がこのペースで解決されている」という実績が
購買動機になるため）。ただしナレッジ本文へのリンクはプレミアムのみで、非プレミアムには
一覧内に登録導線（プレミアムセクションへのボタン）を出す。

### 仕組み

- データ源はサブスクDB（作者のMedical DB）。`由来 = 現場の疑問` のページが対象。
- サブスク同期（`src/app/api/subscription/sync/_core.ts`）が下記プロパティをAlgoliaへ載せ、
  `filterOnly(origin)` でfacet化。取得は `src/lib/resolved-cqs.ts` に集約：
  プレミアム会員は手元のキーで直接Algoliaへ（notionUrl付き）、非プレミアムは公開ティーザー
  API **`GET /api/resolved-cqs`**（認証なし・タイトル/職種/ペンネーム/日付のみ・notionUrlなし・
  CDNキャッシュ10分）経由。
- **ResolvedCqBanner**（`src/components/ResolvedCqs.tsx`）: プレミアム会員がアプリを開いたとき、
  ...
  公開しました。」の形式。×または「一覧を見る」で既読化。非会員には出さない。
- **ResolvedCqHistory**（同ファイル）: 設定 →「解決したみんなの臨床疑問」（全ユーザーに
  メニュー表示）で全件を新しい順に見返せる。プレミアムは各項目からNotionページへジャンプ可、
  非プレミアムはリンクなし＋登録導線カード。
...
4. 以降、プレミアム会員のアプリ起動時にバナー通知 → 既読後も設定内一覧で閲覧可能。
   非プレミアムも設定内一覧（本文リンクなし）で解決実績を見られる
